
/**
 * The Calculations class performs addition, subtraction,
 * multiplication, division, and modulus operations on integers and
 * decimals.
 *  
 * FLVS 2007
 * @author Bill Jordan; A.Ross
 * @version 01/19/07; 06/24/12
 */
public class CalculationsV5
{
    public static void main(String[ ] args)
    {  int iNum1 = 25;
        int iNum2 = 9;
        int iNum3 = 11;
        int iNum4 = 985;
        double dNum1 = 43.21;
        double dNum2 = 3.14;
        double dNum3 = 97.8;
        double dNum4 = 5.34; 
      
        // Addition
        System.out.println("Addition");
        System.out.println( iNum1 +"plus" + iNum2+ "=");
        System.out.println(25 + 9);
        System.out.println(iNum1 + iNum2);
        System.out.println(dNum1 + "plus" + dNum2 + "=");
        System.out.println( 43.21 + 3.14 + 5.0 );
        System.out.println( iNum3 + "plus" + iNum4 +"plus"+ iNum1+ "=");
        System.out.println(11 + 985 + 25);
        System.out.println(dNum3 +"plus" + dNum4 + "=");
        System.out.println(97.8 + 5.34);
        //System.out.println();
        
        // Subtraction
        int iNum5 = 16;
     
  
        System.out.println("Subtraction");
        System.out.println(iNum5 + "minus" + iNum2 + "minus" + iNum1 + "=");
        System.out.println( 16 - 9 - 25 );
        System.out.println( "3.14 - 5.0 equals" + (25 +9));
        System.out.println(dNum3 + "minus" + dNum2 + "=");
        System.out.println(97.8 - 3.14 );
        
        
        //System.out.println();
        
        // Multiplication
        System.out.println("Multiplication");
        System.out.println( 25 * 9 ); 
        System.out.println(3.14 * 5.0 * 5.0 );
        //System.out.println();
        
        // Division
        System.out.println("Division");
        System.out.println( 9 / 25 );
        System.out.println( 43.21 / 5.0 );
        //System.out.println();
         
        // Modulus operator
        System.out.println("Modulus");
        System.out.println( 16 % 9 );
        System.out.println( 5.0 % 3.14 );
        //System.out.println();
        
        // 2.02 Lab Equations
        // 15 divided by 2.5 times -2 plus 10 / 5
        // 234 minus (234 divided by 6 modulus 12) + 3
        // (46.2 divided by 11) minus 3 plus 24 modulus (17 minus 2 times 3)
        // 480 divided by 10 divided by 12 plus 200 *.5 minus 20 modulus 8
        
        // 2.03 Additional int Equations
        
        
        //2.04 Additional double Equations
        
               
    } // end of main method
} // end of class